...
class TestBoardsViews(TestCase):
    ...
    def test_ban_user_view(self):
        ...
        response = self.client.get(
            reverse('ban_user', kwargs={'board': self.board.slug,
                                        'user_id': self.user_to_ban.id}))
        self.assertRedirects(response,
            reverse('banned_users', kwargs={'board': self.board.slug}), status_code=302)
    ...
    def test_user_created_boards_page_view(self):
        ...
        response = self.client.get(
            reverse('user_created_boards', kwargs={'username': self.user.username}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_board_page_view(self):
        ...
        # logged in client
        response = self.client.get(
            reverse('board', kwargs={'board': self.board.slug}))
        self.assertEqual(response.status_code, 200)
